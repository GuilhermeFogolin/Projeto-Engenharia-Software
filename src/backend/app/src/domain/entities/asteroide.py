class Asteroide:
    def __init__(self, id, id_referencia_neo, nome, url_nasa_jpl, magnitude_absoluta_h, diametro_estimado, e_potencialmente_perigoso, dados_aproximacao, e_objeto_sentinela, links):
        self.id = id
        ...
        if self.dados_aproximacao:
            for aproximacao in self.dados_aproximacao:
                dados_aproximacao_traduzido.append({
                    ...
                    "data_aproximacao_completa": aproximacao.get("close_approach_date_full"),
                    "epoch_data_aproximacao": aproximacao.get("epoch_date_close_approach"),
                    "velocidade_relativa": {
                        "quilometros_por_segundo": aproximacao.get("relative_velocity", {}).get("kilometers_per_second"),
                        "quilometros_por_hora": aproximacao.get("relative_velocity", {}).get("kilometers_per_hour"),
                        "milhas_por_hora": aproximacao.get("relative_velocity", {}).get("miles_per_hour")
                    },
                    "distancia_perdida": {
                        "astronomica": aproximacao.get("miss_distance", {}).get("astronomical"),
                        "lunar": aproximacao.get("miss_distance", {}).get("lunar"),
                        "quilometros": aproximacao.get("miss_distance", {}).get("kilometers"),
